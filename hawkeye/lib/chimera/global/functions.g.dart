...
class INTERNALChimeraClientFunctionInvoker extends DefaultInvoker {
  ...
  dynamic invokeClientFunction(String func, List<dynamic> params) {
    if (func == 'targetUpdated') {
      if (params.length != 1) {
        print(
            'ERROR: Function $func requires 1 parameters. ${params.length} was provided instead.');
        return null;
      }
    ...
    if (func == 'snack2') {
      if (params.length != 2) {
        print(
            'ERROR: Function $func requires 2 parameters. ${params.length} was provided instead.');
        return null;
      }
      return ChimeraClientFunctions.snack2(
    ...
    if (func == 'snackConnectionError') {
      ...
      return ChimeraClientFunctions.snackConnectionError(
          params[0] as String, params[1] as String);
    }
    if (func == 'signOut') {
      if (params.length != 0) {
        print(
            'ERROR: Function $func requires 0 parameters. ${params.length} was provided instead.');
        return null;
      }
