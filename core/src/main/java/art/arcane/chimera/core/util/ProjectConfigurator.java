...
public class ProjectConfigurator extends QuillService {
    ...
    private void setupNewService(String projectName) {
        ...
        try {
            ...
            L.flush();
            ProcessBuilder pb = new ProcessBuilder(rootProject.getAbsolutePath() + "/gradlew", "build");
            Process p = pb.start();
            J.attempt(p::waitFor);
            new StreamGobbler(p.getInputStream(), "").start();
            new StreamGobbler(p.getErrorStream(), "ERROR: ").start();
            ...
            L.flush();
            pb = new ProcessBuilder(rootProject.getAbsolutePath() + "/gradlew", "protogen");
            Process p1 = pb.start();
            ...
            L.flush();
            pb = new ProcessBuilder(rootProject.getAbsolutePath() + "/gradlew", "protogenSrc");
            Process p2 = pb.start();
            ...
            L.flush();
            pb = new ProcessBuilder(rootProject.getAbsolutePath() + "/gradlew", "build");
            Process fp = pb.start();
