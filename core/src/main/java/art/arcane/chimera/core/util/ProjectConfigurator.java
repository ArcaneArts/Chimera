...
public class ProjectConfigurator extends QuillService {
    ...
    private void setupNewService(String projectName) {
        ...
        try {
            ...
            L.flush();
            ProcessBuilder pb = new ProcessBuilder("cmd /c call " + rootProject.getAbsolutePath() + "\\gradle", "build");
            Process p = pb.start();
            J.attempt(p::waitFor);
            new StreamGobbler(p.getInputStream(), "").start();
            new StreamGobbler(p.getErrorStream(), "ERROR: ").start();
            ...
            L.flush();
            pb = new ProcessBuilder("cmd /c call " + rootProject.getAbsolutePath() + "\\gradle", "protogen");
            Process p1 = pb.start();
            ...
            L.flush();
            pb = new ProcessBuilder("cmd /c call " + rootProject.getAbsolutePath() + "\\gradle", "protogenSrc");
            Process p2 = pb.start();
            ...
            L.flush();
            pb = new ProcessBuilder("cmd /c call " + rootProject.getAbsolutePath() + "\\gradle", "build");
            Process fp = pb.start();
